...
// Get match requests for a user (both sent and received)
export async function getUserMatchRequests(clerkId: string): Promise<MatchRequestWithDetails[]> {
  try {
    const user = await getUserByClerkId(clerkId)
    if (!user) return []
    ...
    console.error('Error in getUserMatchRequests:', error)
    return []
  }
}
...
// Get pending match requests received by user (for notifications)
export async function getPendingMatchRequests(clerkId: string): Promise<MatchRequestWithDetails[]> {
  try {
    const user = await getUserByClerkId(clerkId)
    if (!user) return []
    ...
    console.error('Error in getPendingMatchRequests:', error)
    return []
  }
}
...
export async function respondToMatchRequest(
...
): Promise<MatchRequestWithDetails> {
  const user = await getUserByClerkId(clerkId)
  if (!user) throw new Error('User not found')

  // Verify user is the opponent and request is pending
  ...
  if (fetchError || !request) {
    throw new Error('Match request not found or unauthorized')
  }
